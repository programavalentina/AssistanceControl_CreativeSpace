...
class ListStudents(models.Model):
    IdListStudents = models.AutoField(primary_key=True)
    FKIdCourse = models.ForeignKey(Course, on_delete=models.CASCADE)
    FKLicence = models.ForeignKey(User, limit_choices_to={'FKLicenceType': 8}, on_delete=models.CASCADE)

class Assistance(models.Model):
    IdAssistance = models.AutoField(primary_key=True)
    FKIdCourse = models.ForeignKey(Course, on_delete=models.CASCADE)
    Date = models.DateField('Date')

class AssistanceList(models.Model):
    IdAssistanceList = models.AutoField(primary_key=True)
    FKLicence = models.ForeignKey(User, limit_choices_to={'FKLicenceType': 8}, on_delete=models.CASCADE)
